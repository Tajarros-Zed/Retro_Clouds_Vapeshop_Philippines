...
$getAllDisposables = "SELECT * FROM products WHERE category_id = '1' GROUP BY product_name ORDER BY $filterType";
$allDisposables = mysqli_query($conn, $getAllDisposables);
$getAllDisposablesSorted = "SELECT * FROM products WHERE category_id = '1' GROUP BY product_name ORDER BY $filterType LIMIT $productStart, $limit";
$allDisposablesSorted = mysqli_query($conn, $getAllDisposablesSorted);
$rows = mysqli_num_rows($allDisposables);
...
<div class="w-full flex items-start justify-start flex-col mt-2 lg:mt-0 lg:items-center lg:justify-center lg:h-full">
    <img src="./assets/dispo_banner.png" class="border border-[#FF1695] h-auto lg:h-full lg:object-cover">
</div>
...
<div
    ...
    <?php
    while ($allDisposablesRow = mysqli_fetch_assoc($allDisposablesSorted)) {
        ?>
        <div id="<?php echo $allDisposablesRow['product_id'] ?>" data-value="<?php echo $allDisposablesRow['product_name'] ?>"
            class='$podkitDisplay disposables w-full h-full flex items-center justify-center flex-col border border-[#33FCFF] cursor-pointer customProd'>
            <img src='./assets/catalog/<?php echo $allDisposablesRow['img_dir'] ?>'
            ...
            <div class='mt-6 w-full p-2 h-auto flex flex-col gap-2 sm:px-4'>
                <h2 class='z-10 text-xs text-[#FF1695] font-black uppercase sm:text-sm customName'>
                    <?php echo $allDisposablesRow['product_name'] ?>
                </h2>
                <p class='z-10 text-xs text-white font-medium sm:text-sm'>₱
                    <?php 
                        if($allDisposablesRow['sale'] != $currentDate) {
                            $price = number_format($allDisposablesRow['price'], 2);
                        } else {
                            $price = number_format($allDisposablesRow['price']*0.78, 2) . " (SALE!)";
                        }
                        echo $price;
                    ?>
                </p>
...
<div class="w-full max-w-full flex flex-1 flex-col items-end justify-end flex-wrap lg:col-span-3">
    <div class="flex gap-2 items-end justify-end w-full mobilemd:gap-4">
        <?php
            for($x = 1; $x <= $pages; $x++) {
        ?>
        <div class='<?php if($x == $page) echo "bg-[#FF1695]" ?> disposable-button customProd disposable-buttons w-[20px] h-[20px] border border-[#FF1695] rounded-full flex items-center justify-center mobilemd:w-[30px] mobilemd:h-[30px]'
...
<script>
    $(document).ready(function () {
        $('.disposable-button').click(function (e) {
            $('#disposables-section').load('catalog-disposables.php', { page: e.target.innerText, limit: <?php echo $limit ?> });
        });
    });
    $('.disposables').click(function (e) {
